...
int main(int argc,char *argv[]){
    ...
    for (long long thread_num = 0; thread_num < Nthreads; thread_num++) {
        pthread_create(&threads[thread_num], NULL, (void*(*)(void*))Random_Walk, &mysql);
    }

    for (long long thread_num = 0; thread_num < Nthreads; thread_num++) {
    	(void)pthread_join(threads[thread_num], &exit_status);
    }
